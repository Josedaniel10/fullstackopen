...
const countWordsInString =
  require("../utils/list_helper.js").countWordsInString;

describe("count words in string", () => {
  test("If the word is found in isolation, 1 is returned", () => {
    assert.strictEqual(countWordsInString("node", "node"), 1);
  });
  ...
  test("If you find a word, you count it and keep searching", () => {
    assert.strictEqual(
      countWordsInString(
        "world",
        "The world of the world is a beautiful world",
      ),
      3,
    );
  });
